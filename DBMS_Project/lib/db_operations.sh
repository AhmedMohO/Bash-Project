list_databases() {
    ...
    find "$DB_DIR" -maxdepth 1 -type d ! -path "$DB_DIR" | awk -F/ '{print "- " $NF}'
}
...
create_database() {
	...
	validate_no_spaces "$db_name" || continue

        validate_db_name "$db_name" || continue
...
connect_database() {
	if ! list_databases; then
		...
		validate_db_name "$db_name" || continue
        	...
        	db_path="$DB_DIR/$db_name"
        	if [[ ! -d "$db_path" ]]; then
            	echo "Database '$db_name' does not exist. Try again."
            	continue
        	fi
        	CONNECTED_DB="$db_name"
        	set_ps1
        	echo "Connected to database '$CONNECTED_DB'."
        	break
    	done
}
